...
uint8_t poll_speed(){
	...
	*/
	...
	if(adc_val < 496)adc_val = 496;
	if(adc_val > 1539)adc_val = 1539;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	//101 => 0 - 60 deg => A+ B- C+
		case 5:
			//A+
			start_pwm_C_N(&htim1, TIM_CHANNEL_1);
			//C+
			start_pwm_C_N(&htim1, TIM_CHANNEL_3);
			//B-
			//stop pwm
		...
		case 3:
			...
			start_pwm_C_N(&htim1, TIM_CHANNEL_1);
			//B+
			start_pwm_C_N(&htim1, TIM_CHANNEL_2);
			//C-
			stop_pwm_C_N(&htim1, TIM_CHANNEL_3);
			//set_pin_as_gpio_high(GPIOB, GPIO_PIN_1);
		break;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	...
	//speed = 25;
	//change Hall sensor emulation ,transition timing..
	static uint16_t ratio = ((1<<16) - 1) / 100;
	TIM2->ARR = (100 - speed) * ratio;
	if(TIM2->ARR == 0)TIM2->ARR = 1;
}
